...
<span id="sId" style="display: none">${sessionId}</span>
                ...
                <div class="detail_content">
                    ...
                    <div class="movie_country">
                        <c:forEach var="country" items="${mv.countryNameList}" varStatus="status">
                            ${country}<c:if test="${fn:length(mv.countryNameList) != status.count}">,</c:if>
                        </c:forEach>
                    </div>
    ...
    <script>
        ...
        let showPageNation = () => {
           ...
           console.log("end : "+ endNum + " max : " + maxReviewCnt);
        ...
        $(document).on('click', '.modify_btn', function () {
            let inputText = $(this).parent().prev().val();
            ...
            $.ajax({
                ...
                headers: {"content-type" : "application/json"},
                dataType:'text',
                data: JSON.stringify({rvId: rvId, rvContent: inputText}),
                success: function (result) {
                    showReviewAll($(".page_clicked").text(), $('.select_r').val());
                },
                error: function (request, error) {
